...
#   2. Claude (vision) looks at the photo and writes a short, fun "fortune"
...
if not ANTHROPIC_API_KEY:
    print("WARNING: ANTHROPIC_API_KEY missing — fortune generation will fail until it's set.", flush=True)

TG_API = f"https://api.telegram.org/bot{TOKEN}"
# Fonts are bundled in the repo (fonts/) rather than relying on system fonts,
# so this works the same on Railway regardless of the base image.
...
# ---------------------------------------------------------------------------
# Claude vision — generate the fortune text from the pet photo
# ---------------------------------------------------------------------------

def generate_fortune(image_bytes):
    """Send the pet photo to Claude and get back a short, fun fortune."""
